...
# A copy of build_tools/format-diff.sh from rocksdb, just replace
...
# acelyc111/encfs.git main branch. Otherwise, we'll check format of the
# uncommitted code only.
if [ -z "$uncommitted_code" ]
then
  # Attempt to get name of acelyc111/encfs.git remote.
  [ "$FORMAT_REMOTE" ] || FORMAT_REMOTE="$(LC_ALL=POSIX LANG=POSIX git remote -v | grep 'acelyc111/encfs.git' | head -n 1 | cut -f 1)"
